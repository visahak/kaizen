...
**Usage**:
```bash
python3 ~/.claw/skills/{skill-name}/scripts/{script_name}.py [arguments]
```
...
**Example**:
```bash
python3 ~/.claw/skills/{skill-name}/scripts/parse_data.py input.json
```
...
**Action**: Check if `~/.claw/skills/{skill-name}/SKILL.md` exists
...
1. Create directory: `~/.claw/skills/{skill-name}/`
2. Write SKILL.md to: `~/.claw/skills/{skill-name}/SKILL.md`
3. If scripts were generated:
   - Create directory: `~/.claw/skills/{skill-name}/scripts/`
   - Write each script to: `~/.claw/skills/{skill-name}/scripts/{script_name}.py`
   - Make scripts executable: `chmod +x ~/.claw/skills/{skill-name}/scripts/*.py`
4. Ensure proper permissions (readable by user)

**Directory Structure**:
```
~/.claw/skills/{skill-name}/
├── SKILL.md
...
**Note**: The skill is saved to the user's home directory (`~/.claw/skills/`) making it available across all projects.
...
**Location**: ~/.claw/skills/{skill-name}/
...
**Files Created**:
```
~/.claw/skills/read-and-parse-file/
├── SKILL.md
└── scripts/
    └── parse_file.py
```
...
- **Cross-Project Availability**: Skills are saved to `~/.claw/skills/` making them available in all your projects.
