...
This skill retrieves relevant entities from a stored knowledge base based on the current task context. It loads all stored entities and presents them to the agent for relevance filtering.

## How It Works

1. The PreToolUse hook fires before each tool call
2. Script reads tool input from stdin (best-effort, ignored beyond logging)
...
5. The agent receives entities as additional context and applies relevant ones
